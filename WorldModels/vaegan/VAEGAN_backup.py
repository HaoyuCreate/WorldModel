port tensorflow.keras as keras
import os
import numpy as np
...
class VAEGAN(tf.keras.Model):
    def __init__(self,lr=0.0001,args=None):
        ...
        keras.layers.Conv2D(filters=self.DEPTH*2, kernel_size=K_SIZE, strides=2, padding='same'),
        keras.layers.BatchNormalization(),
        keras.layers.LeakyReLU(alpha=0.2),

        keras.layers.Conv2D(filters=self.DEPTH*4, kernel_size=K_SIZE, strides=2, padding='same'),
        keras.layers.BatchNormalization(),
        keras.layers.LeakyReLU(alpha=0.2),

        keras.layers.Conv2D(filters=self.DEPTH*8, kernel_size=K_SIZE, strides=2, padding='same'),
        keras.layers.BatchNormalization(),
        keras.layers.LeakyReLU(alpha=0.2),
        
        keras.layers.Flatten(),
        keras.layers.Dense(self.LATENT_DEPTH),
        keras.layers.BatchNormalization(),
        keras.layers.LeakyReLU(alpha=0.2),
        ])
        
        self.mean_net = tf.keras.Sequential([
            tf.keras.layers.InputLayer(input_shape=(LATENT_DEPTH)),
            keras.layers.Dense(self.LATENT_DEPTH,activation="tanh")
            ])
        self.logsigma_net = tf.keras.Sequential([
            tf.keras.layers.InputLayer(input_shape=(LATENT_DEPTH)),
            keras.layers.Dense(self.LATENT_DEPTH,activation="tanh")
            ])
